...
// Määritellään polut
app.get("/", (req, res) => {
  const name = req.query.name || "Vierailija";
...
app.get("/kissat", (req, res) => {
  ...
  const sort = req.query.sort || "ei määritelty";
  console.log(`Ikä ${minAge} ja järjestys ${sort}`);
  res.render("kissat", { ika: minAge, jarjestys: sort, kissaId: null });
});
...
app.get("/kissat/:id", (req, res) => {
  ...
  res.render("kissat", { kissaId: identifier, ika: null, jarjestys: null });
});
